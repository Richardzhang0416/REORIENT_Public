...
J,ϵ,alg,γ,unc,ms,mp,ps,pp=load_data()
...
function MP!(ms::ms_type,mp::mp_type,unc::u_type,ps::ps_type,pp::pp_type)
    ...
    @variable(m, γ0ph[ms.PH,ms.ZH,ms.I0]>=0, Int)  # number of units of technology on energy hub
    @variable(m, γ0pp[ms.PP,ms.ZP,ms.I0]>=0, Int)  # number of units of technology on production platform
    @variable(m, γ0po[ms.PO,ms.ZO,ms.I0]>=0, Int)  # number of units of technology on onshore
    @variable(m, γ0l[ms.L,ms.I0]>=0, Int)          # number of lines
    ...
    @constraint(m, investment_cost, f>=exp10(-6)*sum(mp.π0[i0]*(sum(mp.ci[mp.map_php[p],i0]*x0ph[p,z,i0] for z in ms.ZH for p in ms.PH) # investment cost
                                      ...
                                      +exp10(-6)*sum(mp.π0[i0]*(sum(mp.cfi[mp.map_php[p],i0]*γ0ph[p,z,i0] for z in ms.ZH for p in ms.PH) # fixed capex
                                                                                    +sum(mp.cfi[mp.map_ppp[p],i0]*γ0pp[p,z,i0] for z in ms.ZP for p in ms.PP)
                                                                                    +sum(mp.cfi[mp.map_pop[p],i0]*γ0po[p,z,i0] for z in ms.ZO for p in ms.PO)
                                                                                    +sum(mp.cfi[mp.map_ll[l],i0]*γ0l[l,i0] for l in ms.L)) for i0 in ms.I0)
    ...
    @constraint(m, num_cap_zh[p=ms.PH,z=ms.ZH,i=ms.I], x0ph[p,z,i]<=γ0ph[p,z,i]*mp.xucap[mp.map_php[p]] )
    @constraint(m, num_cap_zp[p=ms.PP,z=ms.ZP,i=ms.I], x0pp[p,z,i]<=γ0pp[p,z,i]*mp.xucap[mp.map_ppp[p]] )
    @constraint(m, num_cap_zo[p=ms.PO,z=ms.ZO,i=ms.I], x0po[p,z,i]<=γ0po[p,z,i]*mp.xucap[mp.map_pop[p]] )
    @constraint(m, num_cap_line[l=ms.L,i=ms.I], x0l[l,i]<=γ0l[l,i]*mp.xucap[mp.map_ll[l]])
    ...
    @constraint(m, cost_operation[i=ms.I], c0[i] == exp10(-6)*(sum(sum((sum((pp.CG+pp.CFuel)/pp.ηG*pG[g,z,h,i] for g in ps.G)+
                                           ...
                                           sum(sum(sum(pp.CZO[h,pp.map_zo[z]]*pZO[b,z,h,i] for b in ps.B)*pp.Ht for h in ps.sH)*pp.WS for z in ps.ZO)))
    ...
    @constraint(m, converter_cap1[z=ps.ZH, s=ps.S, h=ps.Hs[s],i=ms.I], sum(pL[l,h,i] for l in ps.L if pp.AE[pp.map_zhz[z],pp.map_l[l]]==1) <= 0.8*sum(x0[(dcov,z),i] for dcov in ps.Dcov))
    @constraint(m, converter_cap2[z=ps.ZH, s=ps.S, h=ps.Hs[s],i=ms.I], sum(pL[l,h,i] for l in ps.L if pp.AE[pp.map_zhz[z],pp.map_l[l]]==1) >= -0.8*sum(x0[(dcov,z),i] for dcov in ps.Dcov))
    ...
    # */ ----------------------------- CO2 budget ------------------------------------ /* #
    @constraint(m, co2_budget[i=ms.I], (sum(pG[g,z,h,i]*pp.EG/pp.ηG for g in ps.G for h in ps.sH for z in ps.ZP))*pp.Ht*pp.WS <= pp.ΜE)
